...
# Amazon ASIN Scraper as a Flask app with Colab-matched logic
...
def scrape_asins(asins):
    ...
    for asin in asins:
        ...
        if html_com:
            ...
            for th in soup.find_all('th'):
                if 'item weight' in th.get_text(strip=True).lower():
                    ...
                    weight_str = td.get_text(strip=True) if td else None
                    break
            if not weight_str:
                ...
                if db:
                    for li in db.find_all('li'):
                        ...
                        if 'item weight' in txt:
                            parts = txt.split(':')
                            weight_str = parts[1].strip() if len(parts) > 1 else None
                            break
            ...
            if not dims and db:
                for li in db.find_all('li'):
                    ...
                    if 'dimensions' in txt:
                        parts = txt.split(':')
                        dims = parts[1].strip() if len(parts) > 1 else None
                        break
            if dims:
        ...
        if html_in:
            ...
            core_in = soup.select_one(
                '#corePriceDisplay_desktop_feature_div > div.a-section.a-spacing-none.aok-align-center.aok-relative'
            )
            if core_in:
                sp2 = core_in.select_one('span.aok-offscreen')
                pi = sp2.get_text(strip=True).split(' with')[0] if sp2 else None

        usd_value = parse_price_usd(pc)
        inr_value = parse_price_inr(pi)
        used_weight = max(weight_lbs, dim_weight)
        tiers = compute_tiers(usd_value, used_weight) if usd_value else {k: None for k in ['5%','10%','15%','20%','25%']}
        ideal = pick_ideal(tiers, inr_value)
...
def index():
    ...
    return render_template_string(
        '''
        <!doctype html>
        <html><head><title>Amazon ASIN Scraper</title>
        <style>
